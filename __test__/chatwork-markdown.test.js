const { chatworkifyMarkdown } = require("..");

const zws = String.fromCharCode(0x200b); // zero-width-space

test("Simple text", () => {
  expect(chatworkifyMarkdown("hello world")).toBe("hello world\n");
});

test("Escaped text", () => {
  expect(chatworkifyMarkdown("*h&ello>world<")).toBe("*h&ello>world<\n");
});
...
test("Headings", () => {
  ...
  expect(chatworkifyMarkdown(mrkdown)).toBe(chatwork);
});
...
test("Ordered list", () => {
  ...
  const slack = "1.  list\n2.  list\n3.  list\n";
  expect(chatworkifyMarkdown(mrkdown)).toBe(slack);
});
