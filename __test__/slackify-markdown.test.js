const { slackify } = require("..");

const zws = String.fromCharCode(0x200b); // zero-width-space

test("Simple text", () => {
  expect(slackify("hello world")).toBe("hello world\n");
});

test("Escaped text", () => {
  expect(slackify("*h&ello>world<")).toBe("*h&amp;ello&gt;world&lt;\n");
});
...
test("Bold character in word", () => {
  expect(slackify("he**l**lo")).toBe(`he${zws}*l*${zws}lo\n`);
});
...
test("Inline code", () => {
  ...
  const slack = `hello ${zws}\`world\`${zws}\n`;
  expect(slackify(mrkdown)).toBe(slack);
});
